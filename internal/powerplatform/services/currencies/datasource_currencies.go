...
func (d *CurrenciesDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		MarkdownDescription: "Fetches the list of available Dynamics 365 currencies. For more information see [Power Platform Geos](https://learn.microsoft.com/en-us/power-platform/admin/regions-overview)",
...
func (d *CurrenciesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var plan CurrenciesDataSourceModel
	...
	for _, location := range currencies.Value {
		plan.Value = append(plan.Value, CurrencyDataModel{
			ID:              location.ID,
			Name:            location.Name,
			Type:            location.Type,
