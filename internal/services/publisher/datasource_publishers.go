...
func NewPublishersDataSource() datasource.DataSource {
	return &DataSource{
...
func (d *DataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	...
	resp.Schema = schema.Schema{
		MarkdownDescription: "Fetches the list of Dataverse publishers in an environment.",
		Attributes: map[string]schema.Attribute{
			"timeouts": timeouts.Attributes(ctx, timeouts.Opts{
				Read: true,
			}),
			"environment_id": schema.StringAttribute{
				MarkdownDescription: "Id of the Dataverse-enabled environment to fetch publishers from.",
				Required:            true,
			},
			"publishers": schema.ListNestedAttribute{
				MarkdownDescription: "List of Dataverse publishers in the environment.",
				Computed:            true,
...
func (d *DataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	publishers, err := d.PublisherClient.GetPublishers(ctx, config.EnvironmentId.ValueString())
	if err != nil {
		resp.Diagnostics.AddError(fmt.Sprintf("Client error when reading %s", d.FullTypeName()), err.Error())
		return
	}

	config.Publishers = make([]DataSourceModel, 0, len(publishers))
	for i := range publishers {
		var item DataSourceModel
		setPublisherModelFromDto(&item, config.EnvironmentId.ValueString(), &publishers[i])
		config.Publishers = append(config.Publishers, item)
	}

	resp.Diagnostics.Append(resp.State.Set(ctx, &config)...)
}

func setPublisherModelFromDto(model *DataSourceModel, environmentId string, publisher *publisherDto) {
	...
	model.Address = addressModelsFromDto(publisher, nil)
}
