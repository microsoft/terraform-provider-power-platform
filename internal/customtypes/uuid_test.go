...
func TestUnitUUIDConstructors(t *testing.T) {
	...
	if !NewUUIDNull().IsNull() {
		t.Fatal("expected NewUUIDNull to be null")
	}

	if !NewUUIDUnknown().IsUnknown() {
		t.Fatal("expected NewUUIDUnknown to be unknown")
	}

	v := NewUUIDValue(validUUID)
	if v.ValueString() != validUUID {
		t.Fatal("expected value to match input")
	}

	if p := NewUUIDPointerValue(nil); !p.IsNull() {
		t.Fatal("nil pointer should produce null UUID")
	}

	ptr := validUUID
	if p := NewUUIDPointerValue(&ptr); p.ValueString() != validUUID {
		t.Fatal("pointer constructor did not propagate value")
	}
}
...
func TestUnitUUIDValueUUIDValidation(t *testing.T) {
	...
	if !diags.HasError() {
		t.Fatal("expected diagnostics for nil pointer UUID")
	}

	nullValue := NewUUIDNull()
	if _, diags := nullValue.ValueUUID(); !diags.HasError() {
		t.Fatal("expected diagnostics for null ValueUUID")
	}

	unknownValue := NewUUIDUnknown()
	if _, diags := unknownValue.ValueUUID(); !diags.HasError() {
		t.Fatal("expected diagnostics for unknown ValueUUID")
	}
	...
	if !attrResp.Diagnostics.HasError() {
		t.Fatal("expected attribute diagnostics for invalid uuid")
	}
...
func TestUnitUUIDTypeConversions(t *testing.T) {
	...
	if _, ok := val.(UUIDValue); !ok {
		t.Fatal("ValueFromString should return UUIDValue")
	}
	...
	if _, ok := attrVal.(UUIDValue); !ok {
		t.Fatal("expected UUIDValue from terraform conversion")
	}

	badTf := tftypes.NewValue(tftypes.Bool, true)
	if _, err := typ.ValueFromTerraform(ctx, badTf); err == nil {
		t.Fatal("expected error for invalid terraform type")
	}

	if !typ.Equal(UUIDType{}) {
		t.Fatal("UUIDType should be equal to another UUIDType value")
	}

	if !typ.Equal(&UUIDType{}) {
		t.Fatal("UUIDType should consider pointer receivers equal")
	}
}
