...
- All the JSON response for unit test should be stored in .json files. Files should be placed in a folder with a name coresponding to the Unit Test that is being used. Folder name should ommit `UnitTest` in its name. Each Unit Test folder with .json files be stored at `test\resource` or `test\datasource` with all other resource and/or datasource .go files.
